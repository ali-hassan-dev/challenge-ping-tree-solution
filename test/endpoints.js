...
test.serial.cb(
  'GET /api/target/:id - returns 404 for non-existent target',
  function (t) {
    ...
    servertest(server(), url, { encoding: 'json' }, function (err, res) {
      ...
      t.is(
        res.body.error,
        ERROR_MESSAGES.TARGET_NOT_FOUND,
...
test.serial.cb(
  'POST /api/targets - returns 400 for missing required fields',
  function (t) {
    var opts = { method: 'POST', encoding: 'json' }
...
test.serial.cb(
  'POST /api/target/:id - updates target successfully',
  function (t) {
...
test.serial.cb(
  'POST /api/target/:id - updates only provided fields',
  function (t) {
...
test.serial.cb(
  'POST /api/target/:id - returns 404 for non-existent target',
  function (t) {
    var nonExistentId = 'non-existent-id'
...
test.serial.cb(
  'POST /api/target/:id - returns 400 for empty update data',
  function (t) {
    var originalData = Object.assign({}, TEST_DATA.TARGET_SIMPLE)
...
test.serial.cb(
  'POST /api/target/:id - returns 400 for invalid update fields',
  function (t) {
    var originalData = Object.assign({}, TEST_DATA.TARGET_SIMPLE)
...
test.serial.cb(
  'POST /route - rejects visitor not matching geoState criteria',
  function (t) {
    var targetData = Object.assign({}, TEST_DATA.TARGET_BASIC)
...
test.serial.cb(
  'POST /route - rejects visitor not matching hour criteria',
  function (t) {
    var targetData = Object.assign({}, TEST_DATA.TARGET_BASIC)
...
test.serial.cb(
  'POST /route - returns highest value target when multiple match',
  function (t) {
    var targetData1 = Object.assign({}, TEST_DATA.TARGET_SIMPLE)
    var targetData2 = {
      ...
      createTarget(targetData2, function (err, res2) {
        ...
        routeVisitor(visitorInfo, function (err, res) {
          ...
          t.is(
            res.body.url,
            targetData2.url,
            'should return highest value target url'
          )
          t.end()
        })
      })
...
test.serial.cb(
  'POST /route - rejects when target exceeds daily limit',
  function (t) {
    var targetData = {
      url: 'http://example.com',
      value: '0.50',
...
test.serial.cb(
  'POST /route - returns 400 for missing required fields',
  function (t) {
...
test.serial.cb(
  'POST /route - returns 400 for invalid timestamp',
  function (t) {
...
function createTarget (targetData, callback) {
  var opts = { method: 'POST', encoding: 'json' }
  servertest(server(), API_URLS.TARGETS, opts, callback).end(
...
function routeVisitor (visitorInfo, callback) {
  var opts = { method: 'POST', encoding: 'json' }
  servertest(server(), API_URLS.ROUTE, opts, callback).end(
...
function validateTargetFields (t, target, targetData) {
  ...
  t.is(
    ...
    targetData.maxAcceptsPerDay,
    'should have correct maxAcceptsPerDay'
  )
  t.deepEqual(
    target.accept,
    targetData.accept,
    'should have correct accept criteria'
  )
}

function validateTargetsListResponse (t, targets, expectedTargets) {
  var urls = targets.map(function (target) {
    return target.url
  })
  expectedTargets.forEach(function (expectedTarget) {
    t.true(
      urls.includes(expectedTarget.url),
      'should include target: ' + expectedTarget.url
    )
  })
...
function validateTargetUpdate (t, options) {
  ...
  t.is(
    updatedTarget.maxAcceptsPerDay,
    updateData.maxAcceptsPerDay,
    'should have updated maxAcceptsPerDay'
  )
  t.deepEqual(
    updatedTarget.accept,
    originalData.accept,
    'should keep original accept criteria'
  )
  t.is(
...
function validatePartialTargetUpdate (t, options) {
  ...
  t.is(
    updatedTarget.value,
    partialUpdateData.value,
    'should have updated value'
  )
  t.is(
    updatedTarget.maxAcceptsPerDay,
    originalData.maxAcceptsPerDay,
    'should keep original maxAcceptsPerDay'
  )
  t.deepEqual(
    updatedTarget.accept,
    originalData.accept,
