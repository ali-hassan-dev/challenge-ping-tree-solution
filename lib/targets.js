...
function getExistingTarget (targetId, callback) {
  redis.hget(TARGETS_KEY, targetId, function (redisError, targetData) {
    ...
    if (!parsedTarget) {
      return handleError(
        new Error('Invalid existing target data'),
        500,
        callback
      )
    }
...
function processDecisionForTargets (allTargets, visitorData, callback) {
  filterEligibleTargets(
    allTargets,
    visitorData,
    function (error, eligibleTargets) {
      if (error) return callback(error)
...
function validateVisitorInfo (visitorData) {
  if (!visitorData.geoState) {
    return new Error('Missing required field: geoState')
  }
  if (!visitorData.publisher) {
    return new Error('Missing required field: publisher')
  }
  if (!visitorData.timestamp) {
    return new Error('Missing required field: timestamp')
  }
...
function validateTargetData (targetData) {
  ...
  if (!targetData.maxAcceptsPerDay) {
    return new Error('Missing required field: maxAcceptsPerDay')
  }
